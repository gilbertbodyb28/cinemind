...
export default function DeviceConnect({ id, name, startPath, pollPath, codeField, disconnectPath, connected, username, onChange }) {
  const [device, setDevice] = useState(null);
  const [status, setStatus] = useState("idle"); // idle | pending | authorized | expired | denied | invalid | error
    ...
    try {
      ...
      const poll = async () => {
        ...
        try {
          ...
          if (s === "expired" || s === "denied" || s === "invalid") { setStatus(s); return; }
  ...
  return (
    <div className="flex items-center gap-3 flex-wrap">
      ...
      {["expired", "denied", "invalid", "error"].includes(status) && (
        <span data-testid={`${id}-device-status`} className="text-xs text-rose-300 font-mono">
          {status === "expired" ? "Code expired — try again" : status === "denied" ? "Authorization denied" : "Sign-in failed — try again"}
        </span>
      )}
    </div>
  );
