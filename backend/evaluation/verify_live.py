...
def _release_state(row: Dict[str, Any], today: str) -> str:
    # The verified premiere first (providers.premieres): an older series with a
    # coming season is "upcoming" although it first aired years ago.
    raw = str(row.get("premiere_date") or row.get("release_date") or row.get("first_air_date") or "")[:10]
...
def _in_window(row: Dict[str, Any], filters: Dict[str, Any]) -> bool:
    try:
        premiere = str(row.get("premiere_date") or "")
        year = int(premiere[:4]) if len(premiere) >= 4 else int(row.get("year"))
    except (TypeError, ValueError):
        return False
...
async def _upcoming(db, user_id: str) -> List[Dict[str, Any]]:
    ...
    for job in jobs:
        ...
        if not (is_upcoming_job(job) or _window_is_upcoming(filters) or "upcoming" in str(job.get("name") or "").lower()):
            continue
        # As execute_job runs it: a saved job is served by what it asks for.
        ...
        evidence = {
            ...
            "verified_premiere": sum(1 for row in picks if row.get("premiere_date")),
            "upcoming_only": is_upcoming_job(job),
            "warnings": [item.get("code") for item in warnings],
            "sample": [
                f"{row.get('title')} ({row.get('premiere_date') or row.get('release_date') or row.get('year')}"
                f"{' S%s' % row['premiere_season'] if row.get('premiere_season') else ''}) {row.get('media_type')}"
                for row in picks[:10]
            ],
        }
        # An upcoming job passes when every pick has a verified premiere after
        # today inside its window (HANDOFF.md, beslut 2 2026-09-25).
        verified_all = all(row.get("premiere_date") for row in picks) if is_upcoming_job(job) else True
        status = "PASS" if picks and not outside and states.count("released") == 0 and verified_all else "FAIL"
